...
final class BaseLocalDate extends BaseType {

	public final static BaseLocalDate INSTANCE = new BaseLocalDate();

	private BaseLocalDate() {
	}
	...
	void give(Object entity, ODBSMethod method) {
		try {
			method.set().invokeExact(entity, (LocalDate) null);
		} catch (Throwable e) {
	...
	<O, I> void write1(Object entity, ODBSMethod method, ODBSCodec<O, I> codec, O out) throws IOException {
		...
		try {
			value = (LocalDate) method.get().invokeExact(entity);
		} catch (Throwable e) {
			throw new RuntimeException(e);
		}
		if (value != null) {
			codec.writeField(out, method);
			codec.writeLocaleDate(out, value);
		}
	}

	@Override
	<O, I> void read(Object entity, ODBSMethod method, ODBSCodec<O, I> codec, I in) throws IOException {
		final LocalDate value = codec.readLocalDate(in);
		try {
	...
	MethodHandle methodHandle(Method method) throws IllegalAccessException {
		...
		if (method.getParameterCount() == 0) {
			return handler.asType(MethodType.methodType(LocalDate.class, Object.class));
		} else if (method.getParameterCount() == 1) {
			return handler.asType(MethodType.methodType(void.class, Object.class, LocalDate.class));
		} else {
