...
final class ValueChar extends ValueType {

	public final static ValueChar INSTANCE = new ValueChar();
	...
	void give(Object entity, ODBSMethod method) {
		try {
			method.set().invokeExact(entity, (char) 0);
		} catch (Throwable e) {
	...
	<O, I> void write1(Object entity, ODBSMethod method, ODBSCodec<O, I> codec, O out) throws IOException {
		final char value;
		try {
			value = (char) method.get().invokeExact(entity);
		} catch (Throwable e) {
			throw new RuntimeException(e);
		}
		if (value != 0) {
			codec.writeField(out, method);
			codec.writeChar(out, value);
		}
	}
	...
	<O, I> void read(Object entity, ODBSMethod method, ODBSCodec<O, I> codec, I in) throws IOException {
		final char value = codec.readChar(in);
		try {
	...
	MethodHandle methodHandle(Method method) throws IllegalAccessException {
		...
		if (method.getParameterCount() == 0) {
			return handler.asType(MethodType.methodType(char.class, Object.class));
		} else if (method.getParameterCount() == 1) {
			return handler.asType(MethodType.methodType(void.class, Object.class, char.class));
		} else {
